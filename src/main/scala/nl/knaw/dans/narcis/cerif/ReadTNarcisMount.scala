...
/**
 * Iterate through a repository in the mounted store and apply various queries on the `knaw_long` files
 */
object ReadTNarcisMount extends App {
  val xmlNamespace = "http://www.w3.org/XML/1998/namespace"
  val repositoryName = "easy"

  val store = File("/Volumes/narcis-store")
  val repository = store / repositoryName
  val knawLongFiles = for {
